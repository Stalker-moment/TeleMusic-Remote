...
`
            ...
            case 'yttest':
                    const videoUrl = 'https://youtu.be/9AFqO114Xq4';
                ...
                case 'searchplay':
                    if (args.length == 0) return await reply(`Example: ${prefix + command} `)
                    query = args.join(' ')
                    await relay.playing(lol, user.full_name, query, lol.message.from.id.toString())
                    break
            ...
            case 'searchtest':
                ...
                const keyword = 'lovesick AW'; // Kata kunci pencarian
                const maxResults = 5; // Jumlah maksimum hasil pencarian yang ingin ditampilkan
                searchAndCreateThumbnailCanvas(keyword, maxResults);
